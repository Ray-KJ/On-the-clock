  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="flex-1">
            <div className="flex items-start gap-6 mb-6">
              ...
              <div className="flex-1">
                ...
                <p className="text-xl text-secondary font-semibold mb-2">{creator.username}</p>
        ...
        {/* Subscription Tiers */}
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-foreground mb-6">Subscription Tiers</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {mockTiers.map((tier, index) => (
              <Card 
                key={tier.id} 
                className={`relative overflow-hidden ${
                  index === 1 ? 'ring-2 ring-primary shadow-glow' : ''
                ...
                <CardHeader className={index === 1 ? 'pt-12' : ''}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className={`p-2 rounded-full ${
                      ...
                      {index === 2 ? <Crown className="w-4 h-4" /> : <Users className="w-4 h-4" />}
                    </div>
                  </div>
                  <div className="mb-4">
                  ...
                  <CardDescription>{tier.subscriberCount} current subscribers</CardDescription>
                </CardHeader>
                <CardContent>
        ...
        {/* Recent Content */}
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-foreground mb-6">Recent Videos</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <Card key={index} className="bg-gradient-card border-border shadow-card hover:shadow-lg transition-all group cursor-pointer">
                <div className="relative overflow-hidden">
                  ...
                  <Badge 
                    className={`absolute top-2 left-2 ${
                      video.tier === 'Public' ? 'bg-muted' :
                      video.tier === 'Basic Fan+' ? 'bg-primary' :
                      video.tier === 'Super Fan+' ? 'bg-secondary' :
                      'bg-gradient-primary text-white'
                    }`}
                  >
                    {video.tier}
        ...
        <Card className="bg-gradient-card border-border shadow-card">
          <CardHeader>
            <CardTitle className="text-foreground">About {creator.name}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-foreground mb-4">
              Professional dancer and choreographer with over 5 years of experience in the industry. 
              Passionate about sharing exclusive content, teaching new moves, and connecting with fans 
              through interactive experiences.
            </p>
            <p className="text-muted-foreground">
              Join my tier subscription to get access to behind-the-scenes content, dance tutorials, 
              live Q&A sessions, and exclusive 1-on-1 opportunities. Let's dance together! 💃
            </p>
          </CardContent>
        </Card>
      </main>
